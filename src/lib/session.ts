 ...
 */
export async function createSession(userId: string, companyId: string, email: string, role: 'owner' | 'employee'): Promise<void> {
  const token = jwt.sign({ userId, companyId, email, role }, JWT_SECRET, {
    expiresIn: '7d',
  });
